...
import org.eclipse.hawkbit.repository.UserFields;
import org.eclipse.hawkbit.repository.UserRoleFields;
...
public final class PagingUtility {
    ...
    static Sort sanitizeUserSortParam(final String sortParam) {
        if (sortParam == null) {
            // default
            return Sort.by(Direction.ASC, UserFields.ID.getFieldName());
        }
        return Sort.by(SortUtility.parse(UserFields.class, sortParam));
    }

    static Sort sanitizeUserRoleSortParam(final String sortParam) {
        if (sortParam == null) {
            // default
            return Sort.by(Direction.ASC, UserRoleFields.ID.getFieldName());
        }
        return Sort.by(SortUtility.parse(UserRoleFields.class, sortParam));
    }
}
