...
public class SecurityManagedConfiguration {
    ...
    /**
     * Security configuration for the REST management API - HttpBasic login and password reset
     */
    @Configuration
    @Order(349)
    @EnableWebSecurity
    @ConditionalOnClass(MgmtApiConfiguration.class)
    public static class RestLoginSecurityConfigurationAdapter extends WebSecurityConfigurerAdapter {
        ...
        private final String passwordUrl = MgmtRestConstants.PASSWORD_V1_REQUEST_MAPPING + "/**";

        private final String[] routes = {
            MgmtRestConstants.USER_V1_REQUEST_MAPPING + "/login",
            MgmtRestConstants.USER_V1_REQUEST_MAPPING + "/email2fa",
            passwordUrl
        };
        ...
        public FilterRegistrationBean<DosFilter> dosMgmtFilter(final HawkbitSecurityProperties securityProperties) {
            ...
            final FilterRegistrationBean<DosFilter> filterRegBean = dosFilter(null,
                    securityProperties.getDos().getFilter(), securityProperties.getClients());
                    filterRegBean.setUrlPatterns(Arrays.asList(routes));
            filterRegBean.setOrder(DOS_FILTER_ORDER);
        ...
        protected void configure(final HttpSecurity http) throws Exception {
            ...
            HttpSecurity httpSec = http.requestMatchers().antMatchers(routes).and().csrf().disable();
            ...
            httpSec.authorizeRequests()
                .antMatchers(passwordUrl).permitAll()
            ...
            httpSec.addFilterAfter(tenantRequestFilter, RequestHeaderAuthenticationFilter.class);
            httpSec.httpBasic().authenticationEntryPoint(new HttpStatusEntryPoint(HttpStatus.UNAUTHORIZED));
    ...
    /**
     * Security configuration for the REST management API - endpoints with OAuth2 or JWT
     */
    @Configuration
    @Order(350)
    @EnableWebSecurity
    ...
    public static class RestSecurityConfigurationAdapter extends WebSecurityConfigurerAdapter {
        ...
        protected void configure(final HttpSecurity http) throws Exception {
            ...
            httpSec.authorizeRequests()
                .antMatchers(MgmtRestConstants.BASE_SYSTEM_MAPPING + "/admin/**").hasAnyAuthority(SpPermission.SYSTEM_ADMIN)
                .anyRequest().authenticated();
            ...
            if (oidcBearerTokenAuthenticationFilter != null) {
                ...
                // JWT token filter
                httpSec.addFilterBefore(jwtRequestFilter, RequestHeaderAuthenticationFilter.class);
            }
